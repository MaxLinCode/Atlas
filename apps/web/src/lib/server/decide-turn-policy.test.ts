...
import { decideTurnPolicy, type DecideTurnPolicyInput } from "./decide-turn-policy";
...
describe("decideTurnPolicy", () => {
    ...
    expect(
      decideTurnPolicy(
        input(
          ...
          {},
          { rawText: "What do I have tomorrow?", normalizedText: "What do I have tomorrow?", recentTurns: [] }
        )
      )
    ).toMatchObject({
      action: "reply_only",
    ...
    expect(
      decideTurnPolicy(
        input(
          ...
          { missingSlots: ["time"] },
          { rawText: "Schedule gym tomorrow", normalizedText: "Schedule gym tomorrow", recentTurns: [] }
        )
      )
    ).toMatchObject({
    ...
    expect(
      decideTurnPolicy(
        input(
          ...
          { rawText: "Schedule gym tomorrow at 6pm", normalizedText: "Schedule gym tomorrow at 6pm", recentTurns: [] }
        )
      )
    ).toMatchObject({
  ...
  it("recovers and executes when confirmation has one recoverable proposal", () => {
    expect(
      decideTurnPolicy(
        input(
          { turnType: "confirmation", confidence: 0.95, resolvedProposalId: "proposal-1" },
          {},
          {
            rawText: "Yes",
            normalizedText: "Yes",
  ...
  it("treats affirmative consent on a pending proposal as execution", () => {
    expect(
      decideTurnPolicy(
        input(
          { turnType: "confirmation", confidence: 0.97, resolvedEntityIds: ["task-1"], resolvedProposalId: "proposal-1" },
          {},
          {
            rawText: "yes",
            normalizedText: "yes",
  ...
  it("asks for clarification on ambiguous write-like turns", () => {
    expect(
      decideTurnPolicy(
        input(
          { turnType: "edit_request", confidence: 0.55, resolvedEntityIds: ["task-1"] },
          {},
  ...
  it("uses present_proposal only for explicit confirmation-required policy", () => {
    expect(
      decideTurnPolicy(
        input(
          { turnType: "planning_request", confidence: 0.93, resolvedEntityIds: ["task-1"], resolvedProposalId: "proposal-1" },
          {},
          {
  ...
  it("routes ready clarification answers to present_proposal when deterministic consent is still required", () => {
    expect(
      decideTurnPolicy(
        input(
          { turnType: "clarification_answer", confidence: 0.92, resolvedEntityIds: ["task-1"], resolvedProposalId: "proposal-1" },
          {},
          {
            rawText: "3:15pm",
  ...
  it("does not bind consent to a stale proposal with a different target", () => {
    expect(
      decideTurnPolicy(
        input(
          { turnType: "edit_request", confidence: 0.91, resolvedEntityIds: ["task-2"], resolvedProposalId: "proposal-1" },
          {},
          {
