...
export type ConversationResponseDependencies = {
  respond?: (input: BuildConversationResponseInput) => Promise<BuildConversationResponseResult>;
};
...
export async function buildConversationResponse(
  ...
  const respond = dependencies.respond ?? respondToConversationTurnWithResponses;
