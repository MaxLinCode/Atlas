...
vi.mock("@atlas/integrations", async () => {
  const actual = await vi.importActual<typeof import("@atlas/integrations")>(
    "@atlas/integrations",
  );
...
beforeEach(async () => {
  ...
  editTelegramMessageMock.mockImplementation(
...
describe("telegram webhook route", () => {
  it("gates unlinked users before ingress persistence and sends a Google connect link", async () => {
    ...
    expect(sendTelegramMessageMock).toHaveBeenCalledWith({
      ...
      text: expect.stringContaining(
        "I can do that, but I need access to your Google Calendar first. Connect here: https://atlas.example.com/google-calendar/connect?token=",
      ),
    });
  ...
  it("binds numbered followup replies and marks the selected task done", async () => {
    const { store, followUpStore } = await seedOutstandingFollowUpBundle([
  ...
  it("accepts short natural-language followup replies that reference bundle numbers", async () => {
    const { store, followUpStore, taskIds } =
      await seedOutstandingFollowUpBundle(["Task 1", "Task 2"]);
    ...
    expect(
      listTasksForTests().find((task) => task.id === taskIds[1]),
    ).toMatchObject({
      lifecycleState: "done",
    });
  });
  ...
  it("lets mixed-intent messages fall through to the normal router even with outstanding followups", async () => {
    ...
    expect(
      listTasksForTests().filter((task) => task.lifecycleState === "done"),
    ).toHaveLength(0);
  ...
  it("keeps conversation_then_mutation turns non-writing on the first slice", async () => {
    ...
    const conversationResponder = vi.fn(async () => ({
      reply:
        "We can explore tomorrow morning options first, then I can make the change after you confirm.",
    }));
    ...
    const response = await handleTelegramWebhook(
      ...
      }),
      {
        ...
        conversationMemorySummarizer: async () => ({
          summary:
            "The user is discussing a possible move to tomorrow morning.",
        }),
        conversationResponder,
        turnRouter: async () =>
          buildRoutedTurn({
  ...
  it("falls back to recent turns when summary generation fails", async () => {
    ...
    const conversationResponder = vi.fn(async () => ({
      reply:
        "From our recent exchange, it sounds like you want to keep talking through it first.",
    }));
  ...
  it("keeps write-adjacent conversation replies hedged instead of making hard system claims", async () => {
    ...
    expect(editTelegramMessageMock).toHaveBeenCalledWith(
      expect.objectContaining({
        chatId: "999",
        messageId: 88,
        text: "From our recent exchange, it sounds like you mean the dentist reminder, but I am not treating that as confirmed state.",
      }),
    );
  });
