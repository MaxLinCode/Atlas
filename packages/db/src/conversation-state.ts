...
export interface ConversationStateStore {
  loadConversationState(userId: string, transcriptLimit: number): Promise<ConversationStateSnapshot | null>;
...
class InMemoryConversationStateStore implements ConversationStateStore {
  private readonly conversationsByUserId = new Map<string, ConversationRecord>();
  private readonly turnsByConversationId = new Map<string, ConversationTurn[]>();
  private readonly entitiesByConversationId = new Map<string, ConversationEntity[]>();
  private readonly discourseByConversationId = new Map<string, ConversationDiscourseState | null>();

  async loadConversationState(userId: string, transcriptLimit: number): Promise<ConversationStateSnapshot | null> {
    ...
    return buildSnapshot({
      conversation,
      transcript: (this.turnsByConversationId.get(conversation.id) ?? []).slice(-transcriptLimit),
      entityRegistry: this.entitiesByConversationId.get(conversation.id) ?? [],
      discourseState: this.discourseByConversationId.get(conversation.id) ?? null
    });
  }
  ...
  async saveConversationState(input: {
    ...
    const nextConversation = conversationRecordSchema.parse({
      ...
      summaryText: input.summaryText !== undefined ? input.summaryText : existing.summaryText,
    ...
    return buildSnapshot({
      ...
      entityRegistry: this.entitiesByConversationId.get(nextConversation.id) ?? [],
      discourseState: this.discourseByConversationId.get(nextConversation.id) ?? null
    });
  }
...
export class PostgresConversationStateStore implements ConversationStateStore {
  ...
  async saveConversationState(input: {
    ...
    await this.db.transaction(async (tx) => {
      await tx
        ...
        .set({
          ...
          summaryText: input.summaryText !== undefined ? input.summaryText : conversation.summaryText,
      ...
      if (input.entityRegistry !== undefined) {
        await tx.delete(conversationEntities).where(eq(conversationEntities.conversationId, conversation.id));
      ...
      if (input.discourseState !== undefined) {
        await tx.delete(conversationDiscourseStates).where(
        ...
        if (input.discourseState) {
          await tx.insert(conversationDiscourseStates).values({
            conversationId: conversation.id,
            userId: input.userId,
            payload: conversationDiscourseStateSchema.parse(input.discourseState),
            updatedAt: new Date(updatedAt)
          });
        }
      }
    ...
    if (!snapshot) {
      throw new Error(`Conversation state for user ${input.userId} was not persisted.`);
    }
  ...
  private async getLatestConversation(userId: string): Promise<ConversationRecord | null> {
    ...
    return conversationRecordSchema.parse({
      ...
      mode: row.mode === null ? null : conversationRecordModeSchema.parse(row.mode),
      createdAt: row.createdAt.toISOString(),
      updatedAt: row.updatedAt.toISOString()
    });
  }
...
function getDefaultStore(): InMemoryConversationStateStore | PostgresConversationStateStore {
