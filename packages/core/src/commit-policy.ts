import type { ResolvedSlots, SlotKey, TurnInterpretation, WriteContract } from "./index";
...
export function applyCommitPolicy(input: CommitPolicyInput): CommitPolicyOutput {
  const { turnType, extractedValues, confidence, unresolvable, activeContract, priorContract } = input;

  const contractChanged = priorContract != null && activeContract.intentKind !== priorContract.intentKind;
  const priorSlots: ResolvedSlots = contractChanged ? {} : { ...input.priorResolvedSlots };
...
function deriveMissingSlots(contract: WriteContract, committed: ResolvedSlots): SlotKey[] {
