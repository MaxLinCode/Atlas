...
type ProposalOptionEntity = z.infer<typeof conversationProposalOptionEntitySchema>;
...
export function synthesizeMutationText(
  ...
  if (originatingText) {
    const augmentations = buildSlotAugmentations(resolvedSlots, missingSlots, entityRegistry);
    const text = augmentations.length > 0
  ...
  return {
    ...
    reason: "No originating turn text and insufficient resolved slots to synthesize a mutation request."
  };
}
