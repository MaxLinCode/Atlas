...
describe("discourse state", () => {
  it("resolves focus follow-ups through focus_entity_id", () => {
    const afterInitialTurn = updateDiscourseStateFromUserTurn(createEmptyDiscourseState(), {
  ...
  it("resolves editable follow-ups through currently_editable_entity_id", () => {
    const state = updateDiscourseStateFromAssistantTurn(createEmptyDiscourseState(), {
  ...
  it("resolves ordinal references from presented items", () => {
    const state = updateDiscourseStateFromAssistantTurn(createEmptyDiscourseState(), {
      presentedItems: [
        { id: "presented_1", type: "entity", entityId: "gym", ordinal: 1, label: "Gym at 6pm" },
        { id: "presented_2", type: "entity", entityId: "taxes", ordinal: 2, label: "Taxes at 8pm" }
      ]
    }).state;
  ...
  it("keeps multiple pending clarifications independently", () => {
    const withClarifications = updateDiscourseStateFromAssistantTurn(createEmptyDiscourseState(), {
      newClarifications: [
        buildClarification({
          id: "clar-gym",
          entityId: "gym",
    ...
    const resolvedOne = resolvePendingClarification(withClarifications, "clar-gym");
  ...
  it("uses explicit state transitions for clarifying, confirming, editing, and planning", () => {
    const clarifying = updateDiscourseStateFromAssistantTurn(createEmptyDiscourseState(), {
      newClarifications: [
        buildClarification({
          id: "clar-1",
          entityId: "gym",
    ...
    const confirming = updateDiscourseStateFromAssistantTurn(createEmptyDiscourseState(), {
      pendingConfirmation: true
    }).state;
    expect(confirming.mode).toBe("confirming");

    const editing = updateDiscourseStateFromAssistantTurn(createEmptyDiscourseState(), {
      editableEntityId: "taxes"
    }).state;
  ...
  it("cleans up stale pointers, replaces presented items, and bounds recency", () => {
    const prepared = updateDiscourseStateFromAssistantTurn(createEmptyDiscourseState(), {
      presentedItems: [
    ...
    const replacedPresented = updateDiscourseStateFromAssistantTurn(withMentions, {
      presentedItems: [{ id: "presented_3", type: "entity", entityId: "taxes", ordinal: 1 }]
    }).state;
    ...
    expect(withMentions.last_user_mentioned_entity_ids).toEqual(["six", "five", "four", "three", "two"]);
