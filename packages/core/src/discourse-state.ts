...
export type ReferenceResolutionResult = z.infer<
  typeof referenceResolutionResultSchema
>;
...
export function setFocusEntity(
  state: DiscourseState,
...
export function setEditableEntity(
  state: DiscourseState,
  entityId: string | null,
): DiscourseState {
  return discourseStateSchema.parse({
...
export function setPresentedItems(
  state: DiscourseState,
  items: PresentedItem[],
): DiscourseState {
  return discourseStateSchema.parse({
...
export function getActivePendingClarifications(
  state: DiscourseState,
): PendingClarification[] {
  return state.pending_clarifications.filter(
...
export function addPendingClarification(
  ...
  const nextClarifications = [
    ...state.pending_clarifications.filter(
      (existing) => existing.id !== clarification.id,
    ),
    clarification,
  ];
...
export function deriveMode(
  ...
  if (
    hasContractGaps(state) ||
    getActivePendingClarifications(state).length > 0
  ) {
    return "clarifying";
  }
...
export function cleanupDiscourseState(
  ...
  if (!input.validEntityIds) {
    return discourseStateSchema.parse({
      ...state,
      last_user_mentioned_entity_ids:
        state.last_user_mentioned_entity_ids.slice(
          0,
          MAX_LAST_USER_MENTIONED_ENTITY_IDS,
        ),
    });
  }

  const validEntityIds = new Set(input.validEntityIds);
  const pendingClarifications = state.pending_clarifications.map(
    (clarification) => {
      if (
  ...
  return discourseStateSchema.parse({
    ...state,
    focus_entity_id:
      state.focus_entity_id && validEntityIds.has(state.focus_entity_id)
        ? state.focus_entity_id
        : null,
    currently_editable_entity_id:
      state.currently_editable_entity_id &&
      validEntityIds.has(state.currently_editable_entity_id)
        ? state.currently_editable_entity_id
        : null,
    ...
    last_presented_items: state.last_presented_items.reduce<PresentedItem[]>(
      (items, item) => {
        if (item.type === "entity") {
...
export function resolveReference(
  ...
  const ordinalMatch = findPresentedItemByOrdinalOrOption(
    state.last_presented_items,
    ordinalLookupInput,
  );
  ...
  if (input.refersToOther) {
    const otherResolution = resolveOtherPresentedItem(
      state,
  ...
  const activeClarifications =
    getActivePendingClarifications(state).sort(sortClarifications);
  ...
  if (
    activeClarifications.length === 1 &&
    singleClarificationEntityIds.length === 1
  ) {
    return {
      status: "resolved",
  ...
  const recentCandidate = state.last_user_mentioned_entity_ids.find(
    (entityId) =>
      candidateEntityIds.length === 0
        ? true
        : candidateEntityIds.includes(entityId),
  );
...
export function updateDiscourseStateFromUserTurn(
  ...
  const referenceResolution = input.reference
    ? resolveReference(before, input.reference)
    : undefined;
  ...
  for (const clarificationId of input.cancelledClarificationIds ?? []) {
    nextState = discourseStateSchema.parse({
      ...nextState,
      pending_clarifications: nextState.pending_clarifications.map(
        (clarification) =>
  ...
  nextState = cleanupDiscourseState(
    nextState,
    compactObject({
...
export function updateDiscourseStateFromAssistantTurn(
  ...
  if ((input.cancelledClarificationIds ?? []).length > 0) {
    nextState = discourseStateSchema.parse({
      ...
      pending_clarifications: nextState.pending_clarifications.map(
        (clarification) =>
          input.cancelledClarificationIds?.includes(clarification.id) &&
          clarification.status === "pending"
            ? { ...clarification, status: "cancelled" }
            : clarification,
      ),
...
function pruneStaleClarifications(state: DiscourseState): DiscourseState {
  const pending = state.pending_clarifications.filter(
    (c) => c.status === "pending",
  );
...
function findPresentedItemByOrdinalOrOption(
  ...
  return items.find((item) => {
    ...
    if (
      item.type === "option" &&
      input.optionKey &&
      item.optionKey === input.optionKey
    ) {
      return true;
    }
...
function resolveOtherPresentedItem(
  ...
  const excluded =
    candidateEntityIds.find((entityId) => entityId === state.focus_entity_id) ??
    state.focus_entity_id;
...
function sortClarifications(
  left: PendingClarification,
  right: PendingClarification,
) {
