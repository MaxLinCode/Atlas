...
describe("normalizeRawExtraction", () => {
	it("normalizes absolute time to TimeSpec", () => {
		expect(
			normalizeRawExtraction(extraction({ time: { kind: "absolute", hour: 17, minute: 0 } })),
		).toMatchObject({
	...
	it("preserves hour and minute values", () => {
		expect(
			normalizeRawExtraction(extraction({ time: { kind: "absolute", hour: 9, minute: 30 } })),
		).toMatchObject({
	...
	it("drops time when hour is out of range", () => {
		expect(
			normalizeRawExtraction(extraction({ time: { kind: "absolute", hour: 25, minute: 0 } })),
		).toEqual({});
	});

	it("drops time when minute is out of range", () => {
		expect(
			normalizeRawExtraction(extraction({ time: { kind: "absolute", hour: 10, minute: 70 } })),
		).toEqual({});
	});

	it("drops time when hour is negative", () => {
		expect(
			normalizeRawExtraction(extraction({ time: { kind: "absolute", hour: -1, minute: 0 } })),
		).toEqual({});
	});
