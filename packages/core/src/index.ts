...
export function getAppBaseUrl(
  config: Pick<AppConfig, "APP_BASE_URL"> = getConfig(),
) {
...
export function getTelegramAllowedUserIds(
  config: Pick<AppConfig, "TELEGRAM_ALLOWED_USER_IDS"> = getConfig(),
) {
  ...
  if (
    ...
    if (!hasCompleteCurrentCommitment) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message:
          "Scheduled tasks must include a complete current calendar commitment.",
      });
    }
    ...
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      message:
        "Only scheduled or awaiting_followup tasks may retain current calendar commitment fields.",
    });
  }
...
export const scheduleConstraintSchema = z
  ...
  .superRefine((constraint, ctx) => {
    ...
    if (
      constraint.dayReference === "weekday" &&
    ...
    if (constraint.relativeMinutes != null) {
      if (
        ...
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message:
            "relativeMinutes cannot be combined with absolute date or time fields.",
    ...
    if (
      ...
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message:
          "explicitHour or preferredWindow is required when relativeMinutes is null.",
        path: ["explicitHour"],
      });
    }
...
export const conversationProposalOptionEntitySchema =
...
export const conversationScheduledBlockEntitySchema =
  conversationEntityBaseSchema.extend({
...
export const conversationClarificationEntitySchema =
  conversationEntityBaseSchema.extend({
...
export const conversationReminderEntitySchema =
  conversationEntityBaseSchema.extend({
...
export type ScheduleProposalOutput = z.infer<
  typeof scheduleProposalOutputSchema
>;
...
export type ConversationRecordMode = z.infer<
  typeof conversationRecordModeSchema
>;
...
export type TurnInterpretationType = z.infer<
  typeof turnInterpretationTypeSchema
>;
...
export function buildCapturedTask(
  input: CapturedTaskInput,
...
export function buildScheduleAdjustment(input: {
  ...
  const existingDurationMinutes = Math.round(
    (Date.parse(input.block.endAt) - Date.parse(input.block.startAt)) / 60000,
  );
  const durationMinutes =
    computeConstraintDurationMinutes(input.scheduleConstraint) ??
    (existingDurationMinutes || 60);
  const startAt = computeStartAt({
    referenceTime,
    profile: input.userProfile,
    existingBlocks: input.existingBlocks.filter(
      (block) => block.id !== input.block.id,
    ),
...
function computeConstraintDurationMinutes(
  constraint: ScheduleConstraint | null,
) {
  if (
    constraint?.explicitHour == null ||
  ...
  return (
    constraint.endExplicitHour * 60 +
    constraint.endMinute -
    (constraint.explicitHour * 60 + constraint.minute)
  );
}
...
function computeStartAt(input: ComputeStartAtInput) {
  ...
  const constraintDurationMinutes = computeConstraintDurationMinutes(
    input.constraint,
  );
  ...
  if (input.constraint) {
    ...
    const hour =
      input.constraint.explicitHour ??
      preferredWindowHour(input.constraint.preferredWindow);
...
function computeEndAt(input: ComputeStartAtInput) {
  ...
  const durationMinutes =
    computeConstraintDurationMinutes(input.constraint) ??
    input.profile.focusBlockMinutes;
  return new Date(start.getTime() + durationMinutes * 60_000);
}

function advanceForConflicts(
  start: Date,
...
function addDaysToLocalDate(
  ...
  const shifted = new Date(
    Date.UTC(date.year, date.month - 1, date.day + dayCount, 0, 0, 0),
  );
...
function resolveConstraintLocalDate(
  ...
  return addDaysToLocalDate(
    localDate,
    daysUntilWeekday(
      getWeekdayInTimeZone(now, timeZone),
      constraint.weekday,
      constraint.weekOffset,
    ),
  );
}
...
export function detectTaskCalendarDrift(input: {
  ...
  const calendarChanged =
    task.externalCalendarId !== liveEvent.externalCalendarId;
...
export function isTaskFollowupReminderDue(
  task: Task,
