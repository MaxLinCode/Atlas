...
3. An app-layer service loads only the context needed for the current turn, which may be lightweight conversational context, user preferences, and relevant recent schedule information rather than the full task graph.
...
1. When the turn clearly implies side effects, the app prepares a structured planning request with the relevant persisted task and schedule context.
2. The model returns structured planning actions that are validated against app-owned schemas in `packages/core`.
3. The app service resolves symbolic references against persisted Atlas state where needed, then repository-layer persistence records `planner_runs`, creates or updates `tasks` and `schedule_blocks`, and advances the inbox item to the appropriate outcome.
4. Telegram reminder and future follow-up delivery operate from persisted Atlas state rather than transient webhook context.
