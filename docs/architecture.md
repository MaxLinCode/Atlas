...
1. The current chat transport sends a webhook event to `apps/web`.
...
4. Bot-delivered reminders, follow-ups, and future conversational replies operate from persisted Atlas state rather than transient webhook context.
...
6. Google Calendar linking now starts from a one-time chat-to-browser handoff and a short-lived server-side link session before OAuth start, rather than a bearer-style user token on the OAuth-start URL.
...
- Keep the public app surface minimal: messaging webhook, Google linking routes, and explicitly protected cron entrypoints only.
...
- For MVP, the current messaging-platform user IDs are the canonical persisted user identifier; introduce an internal Atlas UUID mapping only when multi-surface identity becomes a real product need.
...
- `docs/architecture/system-boundaries.md`: stack ownership for the messaging transport, Vercel, OpenAI, and Neon Postgres
